...
When you create an app specifically for deploying to CloudHub, Mule's Integration Platform as a Service (iPaaS), there are a few special considerations to take into account. The best way to learn how to develop a CloudHub application is to read the link:/cloudhub/getting-started-with-cloudhub[Getting Started with CloudHub] guide, which includes walkthroughs of basic concepts and examples to help get you familiar with the process of developing and deploying your applications in CloudHub. 
...
* Learn about link:/cloudhub/deploying-an-application-to-arm[Deploying an Application to ARM]
* After you develop and deploy your application, see link:/cloudhub/managing-applications-on-arm[Managing Applications on ARM] at runtime.
