...
.  Deploy an application (either to CloudHub or an on premises server) that includes one or more link:/mule-user-guide/v/3.7/vm-transport-reference[VM endpoints] or that includes a flow set to the queued-asynchronous link:/mule-user-guide/v/3.7/flow-processing-strategies[processing strategy]. 
