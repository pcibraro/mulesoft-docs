...
* Through the *Anypoint Platform* - Log in to the Anypoint Platform (either the online or the on premises edition), select the *Runtime Manager*, in the *Applcations* tab, click the *Deploy application* button.
...
* Directly to your Mule ESB instance On Premises - Place your application's .zip file in the *apps* folder under your Mule home folder. Then configure this mule instance so that it can be managed via ARM.
...
+
[NOTE]
*Note*: For more information on the Applications and Servers tabs, see link:/cloudhub/managing-servers-on-premises[Managing Servers On Premises.]
+
.  A popup will open and instruct you to provide information about the application:
+
...
For more information on deploying to a server or server group, see link:/cloudhub/managing-servers-on-premises[Managing Servers On Premises].
...
After you deploy your application to CloudHub, you can allocate an amount and a size for the *Worker size* of your application. On each application, workers are responsible for executing your application logic.
