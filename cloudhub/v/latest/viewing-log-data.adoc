...
The Anypoint Runtime Manager (ARM) provides access to log data so that you can monitor your application and diagnose any potential issues. ARM stores log data in Universal Time (UTC); however, the console displays the log using your computer's local time zone. 

== Enhanced Log Management

Starting with the August 2015 (r44) release, ARM provides a new optional user interface and expanded storage capabilities for logging. With the new Enhanced Log Management setting, logs can be up to 100 MB or up to 30 days, whichever limit is hit first.
...
* When you enable enhanced logging, CloudHub restarts your application, with link:https://docs.mulesoft.com/cloudhub/managing-applications-on-arm#zero-downtime-updates-with-cloudhub[*zero downtime*].
...
. In the bottom of the ARM Settings screen, click *Apply Changes*.
...
If *Enhanced Log Management* is not selected, ARM saves 100,000 log events per application. Logs that exceed the limit are truncated every 24 hours to 100,000 events by discarding the oldest events past the limit. Individual log entries are limited to a maximum length of 100K characters; longer log entries are truncated to the limit.
...
ARM provides page controls to help you move through the logs when you view historical data. The most recent logs are on page one. The data gets older as you step through the higher pages. 
