...
This tutorial assumes that you are familiar with the concepts covered in the link:/cloudhub/hello-world-on-cloudhub[Hello World Tutorial]. Take a moment to review them if you haven't already done so.
...
|*HTTP Connector* |The HTTP connector (also discussed in the link:/cloudhub/ghello-world-on-cloudhub[Hello World tutorial]) provides a web page for sending Twitter status updates via the URL query string.
...
[TIP]
When deploying to CloudHub, always use host *0.0.0.0* and port `${http.port}` (as discussed in the link:/cloudhub/hello-world-on-cloudhub[Hello World Tutorial] .) The port value lets CloudHub assign a dynamic port when deployed.
