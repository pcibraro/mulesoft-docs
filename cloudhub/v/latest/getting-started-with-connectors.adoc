...
This tutorial assumes that you are familiar with the concepts covered in the link:/cloudhub/getting-started-with-connectors[Hello World Tutorial]. Take a moment to review them if you haven't already done so.

You also need to enable your Twitter account for API access with write privileges. See link:/cloudhub/configure-twitter-for-api-access[Configure Twitter for API Access] for instructions.
