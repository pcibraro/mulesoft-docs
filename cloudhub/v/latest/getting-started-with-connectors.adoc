...
This tutorial follows link:/cloudhub/getting-started-with-connectors[Hello World] as the second installment in the CloudHub Getting Started Guide. It introduces the Twitter Connector, and describes additional CloudHub concepts. After completing this tutorial, you can see how to leverage Anypoint™ Connectors for a wide array of integration solutions with CloudHub.
...
This tutorial assumes that you are familiar with the concepts covered in link:/cloudhub/getting-started-with-connectors[Getting Started with Connectors] and the link:/cloudhub/getting-started-with-connectors[Hello World Tutorial]. Take a moment to review them if you haven't already done so.

You also need to enable your Twitter account for API access with write privileges. See link:/cloudhub/getting-started-with-connectors[Getting Started with Connectors] for instructions on doing so.
...
|*HTTP Connector* |The HTTP connector (also discussed in the link:/cloudhub/getting-started-with-connectors[Hello World tutorial]) provides a web page for sending Twitter status updates via the URL query string.
...
[TIP]
When deploying to CloudHub, always use host *0.0.0.0* and port `${http.port}` (as discussed in the link:/cloudhub/getting-started-with-connectors[Hello World Tutorial] .) The port value lets CloudHub assign a dynamic port when deployed.
...
If you've never registered an application with Twitter before, read link:/cloudhub/configure-twitter-for-api-access[Configure Twitter for API Access] for instructions. Also, be sure to enable *read and write* access as discussed in the configure document, which requires registering your mobile phone with Twitter. You need the read and write permissions, as the default read-only permissions do not allow the Twitter connector to post Tweets to your user stream.
...
* Want to try another tutorial involving Anypoint Connectors? Try the link:/cloudhub/getting-started-with-connectors[Getting Started with Connectors].
* Interested in learning more about the Mule message? Try the link:/cloudhub/getting-started-with-connectors[Getting Started with Connectors].
* Learn more about link:/cloudhub/getting-started-with-connectors[managing your applications on CloudHub].
...
* link:http://mulesoft.github.com/twitter-connector/mule/twitter.html[Twitter Connector Reference]
