...
The `amc_setup` script actually invokes the link:/cloudhub/the-mule-agent[Mule agent] installation script, which has several useful parameters for configuring security and proxies. For details on the available options, see *Installation Options* in link:/mule-agent/v/1.1.1/installing-mule-agent[Installing Mule Agent].
