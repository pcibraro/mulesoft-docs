...
Mule servers communicate to ARM using the link:/runtime-manager/the-mule-agent[the Mule agent]. To add a server to your on-premise ARM, you need to https://s3.amazonaws.com/mulesoft-fileshare/onprem-test/agent-setup-onPrem.jar[download] and install the on-premise version of the agent.
...
For a full description of the steps outlined in this section, see the *Add a Server* section in link:/runtime-manager/managing-servers-on-premises[Managing Servers On Premises].
