...
:keywords: cloudhub, connectors, arm, runtime manager, clusters
...
This guide helps you manage on premises server clusters in the Runtime Manager. An on-premises cluster enables up to 8 servers to participate in a high availability processing unit so that should one server fail, another server takes over processing applications. A cluster can run multiple applications. All servers in a cluster must run the same version of the Mule Runtime. A cluster can only be made up of servers that are not already in a server group or part of another cluster.

The minimum Mule version that supports this feature is Mule version 3.7.0 and newer. The minimum ARM version is August 2015 (R44).

The Runtime Manager lets you create clusters, add servers to a cluster, and delete servers from a cluster.
...
. From each server in the cluster, run the link:/runtime-manager/managing-servers-on-premises[amc_setup] script to register the server with Anypoint Runtime Manager.

== Creating a Cluster in the Runtime Manager

To create a cluster in the Runtime Manager:
...
+
. Click the *Servers* tab and add each server. For more information, see link:/runtime-manager/managing-servers-on-premises[Managing Servers On Premises]
. 
After all servers are added in the Runtime Manager, click *Create Cluster*. Servers can be added to a cluster in any status.
+
...
. Click the names of the servers to add to your cluster, and click *Create Cluster*. Multicast servers can be in any link:/runtime-manager/managing-servers-on-premises[Status State], whereas unicast servers must have a Status of Running. All servers in a cluster must be running the same Mule Runtime version.
+
...
A server can only be removed from a cluster if its link:/runtime-manager/managing-servers-on-premises[Status State] is Running.
...
== Deleting a Cluster From the Runtime Manager

To delete a cluster from the Runtime Manager:

. From the Runtime Manager's *Servers* tab, click the name of a cluster to show details.
...
. In the Runtime Manager's Servers tab, click the name of a cluster to open the details view. 
. Click *Add Servers*.
+
