...
The VM transport can be used for intra-JVM communication between Mule link:/mule-fundamentals/v/3.4/flows-and-subflows[flows]. This transport by default uses in-memory queues but can optionally be configured to use persistent queues. (Note, however, that VM file persistency does not work on clusters.)

[NOTE]
Note that VM queue persistence behaves differently on CloudHub. See the link:/runtime-manager/fabric[Fabric] documentation for more details.
...
Consider setting up your application following a link:/mule-management-console/v/3.4/reliability-patterns[reliability pattern].
