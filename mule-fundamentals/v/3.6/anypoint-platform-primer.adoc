...
. Interested in checking out CloudHub first? Follow the link:/runtime-manager/getting-started-with-runtime-manager[Getting Started with Runtime Manager] guide.
...
|link:/runtime-manager/runtime-manager-insight[Insight] tracks everything your data does in an application deployed to CloudHub. Insight makes information searchable and helps you find and recover from any errors that occurred during message processing.
